...
function send_byte(value)
    ...
    for i = 1, 8 do
        if (bit.isset(value, 0)) then
            --gpio.gpio_config.gpioLedDio.callback(1)
            pio.pin.sethigh(gpio.gpio_config.gpioLedDio.pin)
        else
            --gpio.gpio_config.gpioLedDio.callback(0)
            pio.pin.setlow(gpio.gpio_config.gpioLedDio.pin)
        end
        --gpio.gpio_config.gpioLedClk.callback(0)
        pio.pin.setlow(gpio.gpio_config.gpioLedClk.pin)
        delay()
        --gpio.gpio_config.gpioLedClk.callback(1)
        pio.pin.sethigh(gpio.gpio_config.gpioLedClk.pin)
        delay()
